...
| --- | --- | --- | --- |
| G-6 独立レビュー | 作成を指示した本人以外が挙動を確認する条件を満たす構成が存在しない(最小体制3名未満、かつ外部のレビュアが不在) | ci-strict / post-release-audit | リポジトリを AGPL-3.0 で公開し、コミュニティのレビューを受ける。**公開までの期間は独立レビューが不在のまま進む** |
