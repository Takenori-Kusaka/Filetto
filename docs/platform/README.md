...
| `guard.json` の `enabled` | **`false`** |

**`protectedPatterns` に7件残っていますが、読む主体がいないため効力を持ちません。**
...
**`process.config.json` に上書きを記録しています。** `/process-init` を再実行しても `guard.enabled: false` が保たれます。

```json
"guard": {
  "enabled": false,
  "reason": "序盤フェーズの .claude/ 最適化期間。permissions.deny は空で運用する(ADR-0015 決定1)",
  "reviewBy": "2026-09-30"
}
```

**ただし `permissions.deny` は毎回7件(秘匿ファイルの読み取り)が書き込まれます。** 空で運用する方針のため、**再実行のたびに空へ戻してください。**
