...
**CI(Linux)で実行したところ、手元(Windows)では出なかった2件が出ました。**

| 検出 | 実体 |
| --- | --- |
| `adapters/node.json:13` Secret Keyword | **`secretScan` の検査コマンドの文字列**(`npx secretlint`)。検査そのものの定義 |
| `docs/spikes/S0-001-graphify.md:80` Hex High Entropy String | **プロンプト注入の検証記録に書いた SHA-256**(空文字列のハッシュ) |

**どちらも秘匿情報ではありません。** 方針へパスと理由を書いて除外しました。

**除外はパスと種別で指定します。行番号では指定しません。** 行番号は編集のたびにずれ、**ずれた先の本物を見逃します。** 種別を書けば、同じファイルの他の種別は検査され続けます。

**手元で0件だったものが CI で2件出ました。** 実行環境で結果が変わることは記録に残します。
